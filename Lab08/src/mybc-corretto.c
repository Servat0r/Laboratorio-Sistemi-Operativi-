...
#define PANIC_SYSCALL(expr, res, error)		\
  if ((expr) != (res)) {			\
    perror(error);				\
    exit(1);					\
  }

#define MAX_INPUT_SIZE 1024 /* 1 KB (_PC_PIPE_BUF == 4096 here) */
...
int main(void){
  while (true){
    ...
    if (pid == -1){
      ...
      ssize_t res;
      res = write(pipe1[1], input, strlen(input) + 1);
      if (res < 0) {
	perror("write");
	write(pipe1[1], "quit\n", MAX_INPUT_SIZE);
	wait(&status);
	...
	//output[strlen(output) - 1] = '\0';       // <-------------- perche????
	output[res] = '\0';
#if 0
